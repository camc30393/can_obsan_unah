    ...
    <aside className="side">
      <div className="side-section">
        ...
        <div className="compare-toggle">
          {["detalle", "scoresan", "ranking", "regiones", "comparar", "tabla"].map(t => (
            <button key={t} className={tab === t ? "active" : ""} onClick={() => setTab(t)}>
              {t === "detalle" ? "Detalle" : t === "scoresan" ? "Score SAN" : t === "ranking" ? "Ranking" : t === "regiones" ? "Regiones" : t === "comparar" ? "Comparar" : "Tabla"}
            </button>
          ))}
        </div>
      </div>
      ...
      {tab === "scoresan" && (
        <div className="side-section">
          {window.ScoreCardSAN && <window.ScoreCardSAN data={data} name={selected} />}
          {window.GenderGaps && <window.GenderGaps data={data} name={selected} />}
          {window.ValueChainDiagram && <window.ValueChainDiagram data={data} name={selected} />}
          {window.SimulatorWhatIf && <window.SimulatorWhatIf data={data} name={selected} />}
        </div>
      )}
      {tab === "ranking" && <RankingTab ranking={ranking} indicator={indicator} selected={selected} setSelected={setSelected} fmtV={fmtV} />}
      {tab === "regiones" && <RegionesTab data={data} indicator={indicator} setSelected={setSelected} fmtV={fmtV} />}
  ...
  return (
    <>
      <div className="side-section">
        ...
        <Donut data={[
          { label: "Hombres", val: dep.productores_hombres, color: "#00213D" },
          { label: "Mujeres", val: dep.productores_mujeres, color: "#C8A415" },
          { label: "Organizaciones", val: dep.productores_orgs, color: "#0F766E" },
        ]} total={sexoTotal} />
      </div>
...
// ============================================================
// REGIONES — agregado de departamentos por región del Plan de Nación
// ============================================================
function RegionesTab({ data, indicator, setSelected, fmtV }) {
  const regiones = window.REGIONES_HONDURAS || {};
  const fmt = (v) => indicator.isPct ? window.fmtPct(v, 1) : window.fmtN(v, 0);

  // Calcular agregado por región (suma para conteos, promedio ponderado para porcentajes)
  const rows = Object.entries(regiones).map(([region, deptos]) => {
    const ds = deptos.map(n => data.departamentos[n]).filter(Boolean);
    let value;
    if (indicator.isPct) {
      // Promedio ponderado por productores (proxy de peso poblacional)
      const num = ds.reduce((s, d) => s + (d[indicator.id] || 0) * (d.productores_total || 0), 0);
      const den = ds.reduce((s, d) => s + (d.productores_total || 0), 0);
      value = den > 0 ? num / den : 0;
    } else {
      value = ds.reduce((s, d) => s + (d[indicator.id] || 0), 0);
    }
    const productores = ds.reduce((s, d) => s + (d.productores_total || 0), 0);
    const superficie  = ds.reduce((s, d) => s + (d.superficie_total || 0), 0);
    return { region, deptos, value, productores, superficie };
  }).sort((a, b) => b.value - a.value);
  ...
  return (
    <div className="side-section">
      <h3>Análisis por región
        <window.InfoTip text="Agrupamiento de los 18 departamentos según el ordenamiento territorial del Plan de Nación 2010-2038. Para indicadores porcentuales se calcula un promedio ponderado por número de productores; para indicadores absolutos, una suma simple. Clic en un departamento para ver su detalle." />
      </h3>
      <div className="subtitle">Indicador: {indicator.label.toLowerCase()}.</div>
      {rows.map(r => (
        <div key={r.region} className="region-card">
          <div className="region-head">
            <div className="region-name">{r.region}</div>
            <div className="region-value">{fmt(r.value)}</div>
          </div>
          <div className="region-bar-wrap">
            <div className="region-bar" style={{ width: `${(r.value / maxVal) * 100}%` }}></div>
          </div>
          <div className="region-deptos">
            {r.deptos.map(name => (
              <button key={name} className="region-chip" onClick={() => setSelected(name)} title={`Ver detalle de ${name}`}>
                {name}
              </button>
            ))}
          </div>
          <div className="region-meta">
            <span>{window.fmtN(r.productores)} productores</span>
            <span>·</span>
            <span>{window.fmtN(r.superficie, 0)} ha</span>
          </div>
        </div>
      ))}
    </div>
